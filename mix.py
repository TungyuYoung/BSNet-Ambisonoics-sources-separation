"""
------------------------------------------------
Original author: Francesc Lluis
New author TungYu Dominick Yeung

Function add:
To optimize the model, we introduce mono-audio sh-srir signal for loss calculation. The original code only calculate loss via
clean source original data. However, the signal has been processed by srir-simulation, which introduce differences into
the data.
------------------------------------------------
"""
...
def prepareMUSDB():
    ...
    if subset == 'train':
        ...
        mus = musdb.DB(root=root, is_wav=False, subsets="test")
...
def prepareFuss():
    root = '/home/tungyu/Project/FUSS/'  # path to FUSS ssdata
...
if dataset == 'musdb':
    ...
    while iMix < num_mixes:
        ...
        if (subset == 'train'):
            print("last_test_track: ", last_test_track)
        ...
        if (((vocal_level_db > level_threshold_db) & (drums_level_db > level_threshold_db) & (
            ...
            if (iMixWithSilentSources < num_mixes_with_silent_sources):  # iMixWithSilentSources = 3000
            ...
            if render_room:
                ...
                srir1 = roomSim.simulate()  # shape of srir1 = (44100, 25)
                # print("shape of srir1 = ", srir1.shape)

                x_vocals_ambi = np.zeros((num_samples + srir1.shape[0] - 1, num_sh_channels))  # shape = (308699, 25)
            ...
            norm_pra = np.max(np.abs(x_mix))

            x_mix = x_mix / norm_pra

            x_vocals_only_ambi = x_vocals_ambi / 3 / norm_pra
            x_bass_only_ambi = x_bass_ambi / 3 / norm_pra
            x_drums_only_ambi = x_drums_ambi / 3 / norm_pra

            # x_target = x_vocals_ambi/3
            ...
            x_vocals_only_ambi_scaled = x_vocals_only_ambi * np.iinfo(np.int16).max
            ...
            # Ambisonics single type

            output_path_vocals_ambi = os.path.join(output_prefix_dir, 'vocals_ambi.wav')
            scipy.io.wavfile.write(output_path_vocals_ambi, sampling_rate, x_vocals_only_ambi_scaled.astype(np.int16))

            output_path_drums_ambi = os.path.join(output_prefix_dir, 'drums_ambi.wav')
            scipy.io.wavfile.write(output_path_drums_ambi, sampling_rate, x_drums_only_ambi_scaled.astype(np.int16))
