...
class Roomsimulator:
    ...
    def __init__(self, maxIsOrder=3, maxShOrder=4, receiverMode=ReceiverMode.IDEALSH, fs=48000, c=343.0):
        ...
        self.roomSize = Coordinates([5, 7, 3.5])  # 输入自己想仿真的房间尺寸
        self.sourcePosition = Coordinates([0.1, 0, -.5])  # 声源位置
        self.receiverPosition = Coordinates([0, 0, -.25])  # 麦克风/听众位置
    ...
    def prepareImageSource(self):  # RIR 镜像模型
            ...
            for yOrder in range(-self.maxIsOrder, self.maxIsOrder + 1):
                for zOrder in range(-self.maxIsOrder, self.maxIsOrder + 1):
                    ...
                    if isOrder <= self.maxIsOrder:
                        ...
                        # isList = [(-3, 0, 0, 3), (-2, -1, 0, 3), ..., (3, 0, 0, 3)]
        self.numIs = len(self.isList)  # 63
    ...
    def prepareWallFilter(self):
        ...
        reflectionCoefficients = np.sqrt(1 - self.absorptionCoefficients)  # 反射系数
    ...
    def simulate(self):
        ...
        self.numShChannels = (self.maxShOrder + 1) ** 2  #  根据球谐阶数计算所得的麦克风数
        ...
        for iIs in range(self.numIs):  # 从 iTs 这个列表里读取数据，其中 isList = [(-3, 0, 0, 3), (-2, -1, 0, 3), ..., (3, 0, 0, 3)]
            ...
            relativeIsPositionCartesian = isPositionCartesian - self.receiverPosition.cart  # 相对位置

            self.isPositions[iIs] = Coordinates(relativeIsPositionCartesian)

            if self.randomizeIsPositions:  # False
        ...
        rir = np.zeros(int(self.irLength_s * self.fs))  # 房间冲激响应
        ...
        print("length of srirout = ", len(srirOut))

        return srirOut

    def plotImageSources(self):  # 房间冲激响应，基于镜像声源模型。
    ...
    def convertRt2Absorption(self):
        ...
        elif (self.rtEquation == 'eyring'):  # 依琳公式，基于反射之间无阻碍传播的介质，其特征是漫反射声场。
